...
def get_weight(shape, gain=np.sqrt(2), use_wscale=False, lrmul=1):
    fan_in = int(np.prod(shape[:-1])) # [kernel, kernel, fmaps_in, fmaps_out] or [in, out]
...
def G_style(
    ...
    **kwargs):                                      # Arguments for sub-networks (G_mapping and G_synthesis).
    ...
    num_layers = int(components.synthesis.input_shape[1])
...
def G_mapping(
    ...
    label_size              = 64,            # Label dimensionality, 0 if no labels.
...
def D_basic(
    ...
    label_size          = 64,            # Dimensionality of the labels, 0 if no labels. Overridden based on dataset.
    fmap_base           = 8192,         # Overall multiplier for the number of feature maps.
    fmap_decay          = 1.0,          # log2 feature map reduction when doubling the resolution.
    fmap_max            = 128,          # Maximum number of feature maps in any layer.
    nonlinearity        = 'lrelu',      # Activation function: 'relu', 'lrelu',
